    ...
    static class ThermoProductCustomDrilling
    {
        ...
        public static void AddDrillings(VinylPart part, Product configuredProduct)
        {
            ...
            if (Part.Product == PRODUCT.Panel)
            {
                if (DTYP1 == 1002)
                {
                    AddSingleHole(bottomOffset: INUP1, leftOffset: LINS);
                    AddSingleHole(bottomOffset: INUP1, leftOffset: Width - RINS);
                }

                if (DTYP2 == 1002 && INUP2 != 0)
                {
                    AddSingleHole(bottomOffset: INUP2, leftOffset: LINS);
                    AddSingleHole(bottomOffset: INUP2, leftOffset: Width - RINS);
                }
            }
        ...
        // AddDrillings single hole
        private static void AddSingleHole(double bottomOffset, double leftOffset)
        {
            if (ConfiguredProduct != null)
            {
                if (bottomOffset > 0 && leftOffset > 0 && HDIA > 0 && HoleDepth >0)
                {
                    ConfiguredProduct.Features.AddHoleFromBottomLeft(ApplyTarget.Back, bottomOffset, leftOffset, HDIA/2, HoleDepth);
                }
            }
        }
