    ...
    static class ICB_FilterParts
    {
        ...
        public static List<ICBPart> Filter(List<ICBPart> icbParts)
        {
            ...
            try
            {
                ...
                {
                    if (part.Quantity == 0)  
                        filteredList.Remove(part);
                    else if (!FilterBy_Parameter.All(filterStr => part.Parameter.Contains(filterStr, StringComparison.OrdinalIgnoreCase)))              
                        filteredList.Remove(part);

                    //else if (FilterBy_PartDescription.Any(filterStr => part.PartDescription.Contains(filterStr, StringComparison.OrdinalIgnoreCase)))
                        //filteredList.Remove(part);
                    //else if (FilterBy_CNCCODE.Any(filterStr => part.CNCCODE.Contains(filterStr, StringComparison.OrdinalIgnoreCase)))
                        //filteredList.Remove(part);
                }
